...
def create_sidebar():
   ...
   with st.sidebar:
       ...
       with st.expander("🔧 高级设置"):
           ...
           price_position_filter = st.checkbox("启用价格位置过滤", value=False, help="过滤价格偏离MA太远的信号")
           if price_position_filter:
               price_deviation_pct = st.slider("价格偏离阈值(%)", 1, 10, 3, help="价格偏离MA线的百分比阈值")
           else:
               price_deviation_pct = 0
           
       return timeframe, ma_fast, ma_slow, signal_types, cross_confirm_bars, show_charts, min_volume, price_position_filter, price_deviation_pct
...
def calculate_dual_ma_signals(df: pd.DataFrame, ma_fast: int, ma_slow: int, cross_confirm_bars: int = 2, price_position_filter: bool = False, price_deviation_pct: float = 0) -> Tuple[Optional[dict], int]:
   """计算双MA交叉信号"""
   try:
       candle_count = len(df)
       ...
       # 价格位置过滤
       if price_position_filter and price_deviation_pct > 0:
           # 检查价格是否过度偏离MA线
           fast_deviation = abs(signal_info["price_current"] - signal_info["ma_fast_current"]) / signal_info["ma_fast_current"] * 100
           slow_deviation = abs(signal_info["price_current"] - signal_info["ma_slow_current"]) / signal_info["ma_slow_current"] * 100
           
           if fast_deviation > price_deviation_pct or slow_deviation > price_deviation_pct:
               # 价格偏离过大，不视为有效信号
...
def format_dataframe(df: pd.DataFrame) -> pd.DataFrame:
   ...
   df_formatted["穿越方向"] = df.apply(get_cross_direction, axis=1)  # 新增列
...
def scan_symbols(base: str, symbols: List[str], granularity: str, ma_fast: int, ma_slow: int, signal_types: List[str], cross_confirm_bars: int, min_volume: float = 0, price_position_filter: bool = False, price_deviation_pct: float = 0) -> Tuple[List[dict], dict]:
   """扫描交易对的双MA交叉信号"""
   ...
   with st.spinner("🧮 正在计算交叉信号..."):
       ...
       for symbol in symbols:
           try:
               ...
               signal_info, candle_count = calculate_dual_ma_signals(df, ma_fast, ma_slow, cross_confirm_bars, price_position_filter, price_deviation_pct)
...
def main():
   ...
   # 创建侧边栏并获取参数
   timeframe, ma_fast, ma_slow, signal_types, cross_confirm_bars, show_charts, min_volume, price_position_filter, price_deviation_pct = create_sidebar()
   ...
   with col2:
       ...
       with st.expander("📋 当前设置", expanded=True):
           ...
           if price_position_filter:
               st.write(f"🎯 **价格过滤**: ±{price_deviation_pct}%")
   ...
   with col1:
       if not scan_pressed:
           ...
           **双MA交叉扫描器**专门用于检测两条移动平均线的交叉信号：
           ...
           - 📍 **价格位置**: 显示当前价格相对于双MA的位置
           - 📏 **MA距离**: 显示两条MA线之间的距离百分比
   ...
   if scan_pressed:
       ...
       try:
           ...
           # 执行扫描
           results, scan_stats = scan_symbols(base, symbols, timeframe, ma_fast, ma_slow, signal_types, cross_confirm_bars, min_volume, price_position_filter, price_deviation_pct)
           ...
           if "金叉信号" in signal_types:
               ...
               if golden_crosses:
                   ...
                   # 添加说明
                   st.info(f"💡 **交易建议**: MA{ma_fast}上穿MA{ma_slow}，看涨信号，可考虑做多")
           ...
           if "死叉信号" in signal_types:
               ...
               if death_crosses:
                   ...
                   # 添加说明
                   st.info(f"💡 **交易建议**: MA{ma_fast}下穿MA{ma_slow}，看跌信号，可考虑做空")
